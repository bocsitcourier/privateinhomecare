...
Google Places enrichment coverage (as of May 2026): 796/796 hero images (100%), 782/796 phones (98.2%), 762/796 websites (95.7%). The 83 facilities that had no Google Maps photos (hospices, senior centers, mobile services) now have web-sourced images cached locally in `attached_assets/facility-images/` and served via `express.static`. Two enrichment functions exist: `enrichFacility` (text search) and `enrichFacilityByPlaceId` (direct place ID lookup, more accurate). Admin endpoint: `POST /api/admin/enrich-missing-data`. Script: `scripts/enrich-missing.ts`.
