...
Facility data quality (post-audit cleanup, May 2026): 796/796 facilities have descriptions ≥150 chars (635 rewritten via OpenAI gpt-5-nano through Replit AI Integrations using `scripts/rewrite-descriptions.ts`, with retry-on-too-short and configurable MIN_DESC_LEN/MAX_DESC_LEN). Zero website URLs contain tracking params (utm_, fbclid, gclid, mc_eid, mc_cid). Zero CLOSED_PERMANENTLY facilities are still status='published'. Zero facility FAQs contain placeholder phrasing — phoneless facilities had "our main phone number" replaced with "our website". Image mismatches reduced from 79 → 7 (`scripts/fix-image-mismatch.ts` re-enriches via place ID and verifies the new URL contains the place_id postcondition). The remaining 14 missing phones, 34 missing websites, and 7 missing images are operational constraints — Google Places has no data for those facilities (mostly hospices and mobile services). Traceability: SQL-only fixes are committed in `scripts/audit-remediation-2026-05.sql` (idempotent). Repeatable proof of cleanup state is generated by `scripts/audit-verifier.ts`, which writes timestamped JSON snapshots to `scripts/audit-snapshots/`.

Production deploy (Digital Ocean droplet, Ubuntu 22.04+, 1GB/1vCPU): nginx terminates TLS and proxies `127.0.0.1:5000` to a Node process supervised by systemd. Source-of-truth files: `nginx/privateinhomecaregiver.conf` (nginx vhost), `deploy/privateinhomecare.service` (systemd unit with `Restart=always`, boot persistence, journald logging), `deploy/install-systemd.sh` (one-time installer that disables PM2 and switches to systemd), `scripts/do-deploy.sh` (push → SSH pull → build → reload nginx → restart systemd → 30 s health-check loop, with `set -euo pipefail` so a failed build never kills the live process). Full operational guide: `deploy/README.md`. After any 502, run the diagnostics in that README.
