...
export async function generateNewArticles(): Promise<void> {
  ...
  for (let i = 0; i < missing.length; i++) {
    ...
    for (let attempt = 1; attempt <= 3 && !done; attempt++) {
      try {
        // Check if image was already pre-generated by the image batch script
        const preGenPath = path.join(IMAGES_DIR, `${art.slug}.png`);
        const preGenExists = existsSync(preGenPath);

        // Run content generation and (optionally) image generation in parallel
...
- keywords: include "${art.city} MA" as one keyword, 10 total keywords`
        ...
        // Process image — prefer pre-generated AI image, then DALL-E download, then no image
        let heroImageUrl = '/attached_assets/ai_generated/placeholder-caregiver.png';

        if (preGenExists) {
          // Best case: image was pre-generated by the batch image script
          heroImageUrl = `/attached_assets/ai_generated/${art.slug}.png`;
          log(`[NewArticleGen]   Using pre-generated image for ${art.slug}`);
        } else if (imgResult.status === 'fulfilled') {
          ...
          if (tempUrl) {
            try {
              ...
              log(`[NewArticleGen]   AI image saved: ${filename}`);
            } catch (dlErr: any) {
              log(`[NewArticleGen]   Image download failed, no image set: ${dlErr.message?.substring(0, 60)}`);
              imgFailed++;
            }
          }
        } else {
          // DALL-E not available — image will be set later by batch script
          log(`[NewArticleGen]   No image yet (will be set by image batch): ${imgResult.reason?.message?.substring(0, 60)}`);
          imgFailed++;
        }
