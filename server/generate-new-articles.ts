...
const BODY_SYSTEM = `You are a professional SEO content writer for Private InHome CareGiver, a Massachusetts in-home care and nurse concierge agency. Today's date is Spring 2026.
...
export async function generateNewArticles(): Promise<void> {
  ...
  const CONCURRENCY = 5;
  log(`[NewArticleGen] ${missing.length} articles to generate (${manifest.length - missing.length} already exist) — running ${CONCURRENCY} parallel workers.`);
  ...
  // Split into round-robin queues so each worker gets an even spread of topics
  const queues: NewManifestItem[][] = Array.from({ length: CONCURRENCY }, () => []);
  missing.forEach((art, i) => queues[i % CONCURRENCY].push(art));

  // Single article generator (used by each worker)
  async function generateOne(art: NewManifestItem, workerId: number): Promise<void> {
    let done = false;

    for (let attempt = 1; attempt <= 3 && !done; attempt++) {
      try {
        const preGenPath = path.join(IMAGES_DIR, `${art.slug}.png`);
        const preGenExists = existsSync(preGenPath);

        const [bodyResult, metaResult] = await Promise.allSettled([
...
- keywords: include "${art.city} MA" as one keyword, 10 total keywords`
        ...
        const heroImageUrl = preGenExists
          ? `/attached_assets/ai_generated/${art.slug}.png`
          : '/attached_assets/ai_generated/placeholder-caregiver.png';
        ...
        const excerpt = meta.excerpt || `Comprehensive guide to ${art.title} for Massachusetts families. Expert advice on senior care options.`;
        ...
        log(`[W${workerId}] ✓ [${completed}/${total}] ${art.slug} | ${wordCount}w | FAQ:${hasFAQ} | Tags:${hasTags} | Links:${hasLinks}`);
      } catch (err: any) {
        const msg = err.message?.substring(0, 100) || 'unknown error';
        log(`[W${workerId}] Attempt ${attempt} failed for ${art.slug}: ${msg}`);
        if (attempt < 3) {
          await new Promise(r => setTimeout(r, attempt === 1 ? 8000 : 15000));
        }
      }
    }

    if (!done) {
      log(`[W${workerId}] PERMANENT FAIL: ${art.slug}`);
      failed++;
      completed++;
    }
  }

  // Worker function — processes its queue sequentially
  async function worker(workerId: number, queue: NewManifestItem[]): Promise<void> {
    log(`[W${workerId}] Starting — ${queue.length} articles assigned`);
    for (const art of queue) {
      await generateOne(art, workerId);
      // Small stagger between articles within same worker
  ...
  log(`[NewArticleGen] Created: ${success} | Failed: ${failed}`);
